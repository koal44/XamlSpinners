...
using MS.Internal.Data2;
using MS.Utility2;           // FrugalList
    ...
    public sealed class PropertyPath
    {
        ...
        private void PrepareSourceValueInfo(ITypeDescriptorContext typeDescriptorContext)
        {
            ...
            if (_arySVI.Length == 0)
            {
                ...
                if (detail == null)
                    detail = Path;
                throw new InvalidOperationException($"PropertyPathSyntaxError, {detail}");
            }
        ...
        object ResolvePropertyName(string name, object item, Type ownerType, object context, bool throwOnError)
        {
            ...
            if (IsParameterIndex(name, out index))
            {
                if (0 <= index && index < PathParameters.Count)
                {
                    ...
                    if (!IsValidAccessor(accessor))
                        throw new InvalidOperationException($"PropertyPathInvalidAccessor, {(
                                    (accessor != null) ? accessor.GetType().FullName : "null")}");

                    return accessor;
                }
                else if (throwOnError)
                    throw new InvalidOperationException($"PathParametersIndexOutOfRange, {index}, {PathParameters.Count}");
                else return null;
            }
            ...
            if (ownerType != null)
            {
                ...
                if (accessor == null && throwOnError)
                    throw new InvalidOperationException($"PropertyPathNoProperty, {ownerType.Name}, {propertyName}");
            ...
            for (int i = 0; i < args.Length; ++i)
            {
                                        ...
                                        out index))
                    {
                        if (0 <= index && index < PathParameters.Count)
                        {
                            ...
                            else if (throwOnError)
                            {
                                ...
                                throw new InvalidOperationException($"PathParameterIsNull, {index}");
                            }
                        }
                        else if (throwOnError)
                            throw new InvalidOperationException($"PathParametersIndexOutOfRange, {index}, PathParameters.Count");
                ...
                else
                {
                    ...
                    if (args[i].type != null)
                    {
                        ...
                        {
                            if (throwOnError)
                                throw new InvalidOperationException($"PropertyPathIndexWrongType, {paramList[i].parenString}, {paramList[i].valueString}");
                            args[i].type = null;
                        }
                    }
                    else
...
#pragma warning restore 1634, 1691
        ...
        Type GetTypeFromName(string name, object context)
        {
            ...
            if (parserContext != null)
            {
                ...
                {
                    throw new ArgumentException($"PropertyPathNoProperty, {nsPrefix}, {name}");
                }
        ...
        static string GetPropertyName(object accessor)
        {
            ...
            {
                Debug.Assert(false, "Unknown accessor type");
                return null;
            }
        }
