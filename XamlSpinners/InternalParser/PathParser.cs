...
{
    public enum SourceValueType { Property, Indexer, Direct };
    public enum DrillIn { Never, IfNeeded, Always };

    public struct SourceValueInfo
    {
        public SourceValueType type;
        public DrillIn drillIn;
        public string? name;                 // the name the user supplied - could be "(0)"
        public List<IndexerParamInfo>? paramList;    // params for indexer
        ...
        public SourceValueInfo(SourceValueType t, DrillIn d, List<IndexerParamInfo> list)
        {
            type = t;
            drillIn = d;
            name = null;
    ...
    public struct IndexerParamInfo
    {
    ...
    public class PathParser
    {
        ...
        void SetError(string id, params object[] args) { _error = $"error: {id}, {args}"; }

        enum State { Init, DrillIn, Prop, Done };

        public (DependencyObject finalTarget, DependencyProperty finalTargetProperty) ResolveFinalAnimationTargetAndProperty(DependencyObject target, PropertyPath path)
        {
        ...
        public SourceValueInfo[] Parse(string path)
        {
            ...
            if (_n == 0)
            {
                ...
                return new SourceValueInfo[] { new SourceValueInfo(SourceValueType.Direct, DrillIn.Never, n: null) };
            }
                    ...
                    case State.DrillIn:
                        switch (c)
                        {
                            ...
                            case NullChar:
                                break;
                            default:
                                SetError("PathSyntax", _path[.._index], _path[_index..]);
                                return EmptyInfo;
                        }
                        _state = State.Prop;
                        break;
        ...
        void AddProperty()
        {
            ...
            if (level > 0)
            {
                SetError("UnmatchedParen", _path![start..]);
                return;
            }

            if (level < 0)
            {
                SetError("UnmatchedParen", _path![.._index]);
                return;
            }

            string name = _path![start.._index].Trim();

            SourceValueInfo info = (name.Length > 0)
                ? new SourceValueInfo(SourceValueType.Property, _drillIn, name)
                : new SourceValueInfo(SourceValueType.Direct, _drillIn, n: null);
        ...
        void AddIndexer()
        {
            ...
            var parenStringBuilder = new StringBuilder();
            var valueStringBuilder = new StringBuilder();

            var paramList = new List<IndexerParamInfo>();
            ...
            while (state != IndexerState.Done)
            {
                if (_index >= _n)
                {
                    SetError("UnmatchedBracket", _path![(start - 1)..]);
                    return;
                }
            ...
            var info = new SourceValueInfo(SourceValueType.Indexer, _drillIn, paramList);
            _al.Add(info);
        ...
        int _n;
        DrillIn _drillIn;
        readonly ArrayList _al = new();
        const char NullChar = Char.MinValue;
        const char EscapeChar = '^';
        static readonly SourceValueInfo[] EmptyInfo = Array.Empty<SourceValueInfo>();
