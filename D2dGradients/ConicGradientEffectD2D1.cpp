...
namespace conicgrad {
    ...
    IFACEMETHODIMP
        ...
        ID2D1ResourceTexture* tex = CreateGradientTexture();
    ...
    IFACEMETHODIMP
        ConicGradientEffectD2D1::QueryInterface(REFIID riid, void** aPtr) {
        if (!aPtr) {
            return E_POINTER;
        }
    ...
    HRESULT
        ...
        if (FAILED(hr)) {
            std::cout << "Failed to register conic gradient effect.";
        }
        return hr;
    }
    ...
    HRESULT __stdcall ConicGradientEffectD2D1::CreateEffect(IUnknown** aEffectImpl) {
    ...
    ID2D1ResourceTexture*
        ...
        if (FAILED(hr)) {
            std::cout << "Failed to create resource texture: " << hr;
        }
